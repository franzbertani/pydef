...
#define sense_temp_output_var struct sense_temp_var_struct {	\
	    int version_array[1 + 1];	\
	    short int versions_count;	\
	    short int window_begin_index;	\
	    short int write_index;	\
	...
	struct sense_temp_var_struct __attribute__ ((persistent)) var_struct_sense_temp = {.versions_count = 1, .window_begin_index = 0, .write_index = 0, .is_full = 0, .write_count=9};	\
	int g_sense_temp;	\
		\
	
#define temp_avg_output_var struct temp_avg_var_struct {	\
	    int version_array[1 + 1];	\
	    short int versions_count;	\
	    short int window_begin_index;	\
	    short int write_index;	\
	...
	struct temp_avg_var_struct __attribute__ ((persistent)) var_struct_temp_avg = {.versions_count = 1, .window_begin_index = 0, .write_index = 0, .is_full = 0, .write_count=9};	\
	int g_temp_avg;	\
		\
...
#define BEGIN_TASK_send void send(){	\
	...
	siren_command("START_CCOUNT: task\n");
#define BEGIN_TASK_sense_temp void sense_temp(){	\
	siren_command("START_CCOUNT: task\n");
#define BEGIN_TASK_temp_avg void temp_avg(){	\
	int sense_temp = g_sense_temp;	\
	siren_command("START_CCOUNT: task\n");
...
#define RETURN_lowpass g_lowpass = lowpass_out;	\
	...
	if(!(task_struct_compress.isEnabled[task_struct_compress_enVersion]) && (app_struct_app_3.isActive[app_struct_app_3.isActiveVersion]) && (1)){	\
	...
	if(app_struct_app_3.isActive[app_struct_app_3.isActiveVersion]) { task_struct_compress.deadline[!task_struct_compress.deadlineVersion & 0x1] = 2000000; task_struct_compress.deadlineVersion = !task_struct_compress.deadlineVersion & 0x1;}	\
...
#define RETURN_operate g_operate = operate_out;	\
	...
	slack=0.2*slack + value;	\
		\
	if (value < 0 || slack < 0){	\
	    if(under_counter[0]<8){	\
	        under_counter[0]++;	\
	        over_counter[0] = 0;	\
	    } else {	\
	        under_counter[0] = 0;	\
...
#define RETURN_compress g_compress = compress_out;	\
	...
	slack-=delta_cycles;	\
	if(app_struct_app_3.isActive[app_struct_app_3.isActiveVersion]) { task_struct_compress.deadline[!task_struct_compress.deadlineVersion & 0x1] = 2000000; task_struct_compress.deadlineVersion = !task_struct_compress.deadlineVersion & 0x1;}extern task_struct_t task_struct_send;	\
	short int task_struct_send_enVersion = task_struct_send.isEnabledVersion;	\
	if(!(task_struct_send.isEnabled[task_struct_send_enVersion]) && (app_struct_app_3.isActive[app_struct_app_3.isActiveVersion]) && (1)){	\
	...
	if(app_struct_app_3.isActive[app_struct_app_3.isActiveVersion]) { task_struct_send.deadline[!task_struct_send.deadlineVersion & 0x1] = task_struct_compress.deadline[!task_struct_compress.deadlineVersion & 0x1]; task_struct_send.deadlineVersion = !task_struct_send.deadlineVersion & 0x1;}	\
...
#define RETURN_send g_send = send_out;	\
	...
	siren_command("PRINTF: updating tput after send, value=%l\n", value);	\
		...
		\
	if (value < 0 || slack < 0){	\
	    if(under_counter[2]<8){	\
	        under_counter[2]++;	\
	        over_counter[2] = 0;	\
	    } else {	\
	        under_counter[2] = 0;	\
	        siren_command("PRINTF: app_3 underperforming\n");	\
	        app_struct_app_3.x_ok[!app_struct_app_3.x_okVersion & 0x1] = -1;	\
	        app_struct_app_3.x_okVersion = !app_struct_app_3.x_okVersion & 0x1;	\
	        manage_underperf();	\
	    }	\
	} else if (value > 0 && slack>0) {	\
	    if(over_counter[2]<2){	\
	        ...
	        over_counter[2]  = 0;	\
	        siren_command("PRINTF: app_3 overperforming\n");	\
	        app_struct_app_3.x_ok[!app_struct_app_3.x_okVersion & 0x1] = 1;	\
	        app_struct_app_3.x_okVersion = !app_struct_app_3.x_okVersion & 0x1;	\
	        if(selected_app<APPS_COUNT-1) selected_app++;	\
	        manage_overperf();	\
	    ...
	    siren_command("PRINTF: app_3 performance ok\n");	\
	    app_struct_app_3.x_ok[!app_struct_app_3.x_okVersion & 0x1] = 0;	\
	    app_struct_app_3.x_okVersion = !app_struct_app_3.x_okVersion & 0x1;	\
	}	\
	if(app_struct_app_3.isActive[app_struct_app_3.isActiveVersion]) { task_struct_send.deadline[!task_struct_send.deadlineVersion & 0x1] = task_struct_compress.deadline[!task_struct_compress.deadlineVersion & 0x1]; task_struct_send.deadlineVersion = !task_struct_send.deadlineVersion & 0x1;}
#define RETURN_sense_temp g_sense_temp = sense_temp_out;	\
	var_struct_sense_temp.version_array[var_struct_sense_temp.write_index] = g_sense_temp;	\
		\
	var_struct_sense_temp.write_index = (var_struct_sense_temp.write_index + 1) % (var_struct_sense_temp.versions_count + 1);	\
		\
	if(var_struct_sense_temp.is_full){	\
	    var_struct_sense_temp.window_begin_index = (var_struct_sense_temp.window_begin_index + 1) % (var_struct_sense_temp.versions_count + 1);	\
	} else if(var_struct_sense_temp.write_index == 0){	\
	    var_struct_sense_temp.is_full = 1;	\
	    var_struct_sense_temp.window_begin_index = (var_struct_sense_temp.window_begin_index + 1) % (var_struct_sense_temp.versions_count + 1);	\
	}	\
	if(var_struct_sense_temp.write_count < var_struct_sense_temp.versions_count){	\
	    var_struct_sense_temp.write_count++;	\
	}	\
		\
	siren_command("GET_CCOUNT: task-%l\n", &delta_cycles);	\
	siren_command("TEST_EXECUTION_CCOUNT: %l, sense_temp\n", delta_cycles);	\
	slack-=delta_cycles;	\
	if(app_struct_app_2.isActive[app_struct_app_2.isActiveVersion]) { task_struct_sense_temp.deadline[!task_struct_sense_temp.deadlineVersion & 0x1] = 1000000; task_struct_sense_temp.deadlineVersion = !task_struct_sense_temp.deadlineVersion & 0x1;}extern task_struct_t task_struct_temp_avg;	\
	short int task_struct_temp_avg_enVersion = task_struct_temp_avg.isEnabledVersion;	\
	if(!(task_struct_temp_avg.isEnabled[task_struct_temp_avg_enVersion]) && (app_struct_app_2.isActive[app_struct_app_2.isActiveVersion]) && (1)){	\
	    task_struct_temp_avg.isEnabled[!task_struct_temp_avg_enVersion & 0x1] |= 0x1;	\
	    enabled_task_array[enabled_task_count] = &task_struct_temp_avg;	\
	    enabled_task_count++;	\
	    task_struct_temp_avg.isEnabledVersion = !task_struct_temp_avg_enVersion & 0x1;	\
	}	\
	if(app_struct_app_2.isActive[app_struct_app_2.isActiveVersion]) { task_struct_temp_avg.deadline[!task_struct_temp_avg.deadlineVersion & 0x1] = task_struct_sense_temp.deadline[!task_struct_sense_temp.deadlineVersion & 0x1]; task_struct_temp_avg.deadlineVersion = !task_struct_temp_avg.deadlineVersion & 0x1;}	\
	
#define RETURN_temp_avg g_temp_avg = temp_avg_out;	\
	var_struct_temp_avg.version_array[var_struct_temp_avg.write_index] = g_temp_avg;	\
		\
	var_struct_temp_avg.write_index = (var_struct_temp_avg.write_index + 1) % (var_struct_temp_avg.versions_count + 1);	\
		\
	if(var_struct_temp_avg.is_full){	\
	    var_struct_temp_avg.window_begin_index = (var_struct_temp_avg.window_begin_index + 1) % (var_struct_temp_avg.versions_count + 1);	\
	} else if(var_struct_temp_avg.write_index == 0){	\
	    var_struct_temp_avg.is_full = 1;	\
	    var_struct_temp_avg.window_begin_index = (var_struct_temp_avg.window_begin_index + 1) % (var_struct_temp_avg.versions_count + 1);	\
	}	\
	if(var_struct_temp_avg.write_count < var_struct_temp_avg.versions_count){	\
	    var_struct_temp_avg.write_count++;	\
	}	\
		\
	siren_command("GET_CCOUNT: task-%l\n", &delta_cycles);	\
	siren_command("TEST_EXECUTION_CCOUNT: %l, temp_avg\n", delta_cycles);	\
	slack-=delta_cycles;	\
	long int value = task_struct_temp_avg.deadline[task_struct_temp_avg.deadlineVersion] - delta_cycles;	\
		\
	siren_command("PRINTF: updating tput after temp_avg, value=%l\n", value);	\
	 	\
		\
	if (value < 0 || slack < 0){	\
	    if(under_counter[1]<8){	\
	        under_counter[1]++;	\
	        over_counter[1] = 0;	\
	    } else {	\
	        under_counter[1] = 0;	\
...
#define TASKS_STRUCTS task_struct_t __attribute__ ((persistent)) task_struct_sense_temp = {.e_wc = 16, .in_set = {}, .in_set_count = 0, .function_pointer = &sense_temp, .isEnabled = {0x0,0x0}, .isEnabledVersion = 0x0, .isActive = {0x0,0x0}, .isActiveVersion = 0x0, .deadlineVersion = 0x0, .marked_to_remove = {0x0,0x0}, .stopped=0, .app_pointer=NULL};	\
	task_struct_t __attribute__ ((persistent)) task_struct_temp_avg = {.e_wc = 16, .in_set = {&task_struct_sense_temp}, .in_set_count = 1, .function_pointer = &temp_avg, .isEnabled = {0x0,0x0}, .isEnabledVersion = 0x0, .isActive = {0x0,0x0}, .isActiveVersion = 0x0, .deadlineVersion = 0x0, .marked_to_remove = {0x0,0x0}, .stopped=0, .app_pointer=NULL};	\
	task_struct_t __attribute__ ((persistent)) task_struct_sense = {.e_wc = 81, .in_set = {}, .in_set_count = 0, .function_pointer = &sense, .isEnabled = {0x0,0x0}, .isEnabledVersion = 0x0, .isActive = {0x0,0x0}, .isActiveVersion = 0x0, .deadlineVersion = 0x0, .marked_to_remove = {0x0,0x0}, .stopped=0, .app_pointer=NULL};	\
	...
	task_struct_t __attribute__ ((persistent)) task_struct_compress = {.e_wc = 16, .in_set = {&task_struct_lowpass}, .in_set_count = 1, .function_pointer = &compress, .isEnabled = {0x0,0x0}, .isEnabledVersion = 0x0, .isActive = {0x0,0x0}, .isActiveVersion = 0x0, .deadlineVersion = 0x0, .marked_to_remove = {0x0,0x0}, .stopped=0, .app_pointer=NULL};	\
...
#define TASK_ARRAY task_struct_t* __attribute__ ((persistent)) task_array[10] = {&task_struct_sense, &task_struct_median, &task_struct_lowpass, &task_struct_magnitude, &task_struct_classify, &task_struct_operate, &task_struct_compress, &task_struct_send, &task_struct_sense_temp, &task_struct_temp_avg};	\
	task_struct_t* __attribute__ ((persistent)) active_task_array[10] = {};	\
	int __attribute__ ((persistent)) active_task_count = 0;	\
	task_struct_t* __attribute__ ((persistent)) enabled_task_array[10] = {};	\
...
#define APP_STRUCTS app_struct_t __attribute__ ((persistent)) app_struct_app_1 = {.x_min = 333333, .x_ok = {0,0}, .x_okVersion = 0x0, .tasks_count = 6, .app_tasks = {&task_struct_sense, &task_struct_median, &task_struct_lowpass, &task_struct_magnitude, &task_struct_classify, &task_struct_operate}, .initial_task = &task_struct_sense, .final_task = &task_struct_operate, .isActive = {0x0, 0x0}, .isActiveVersion = 0x0};	\
	app_struct_t __attribute__ ((persistent)) app_struct_app_2 = {.x_min = 500000, .x_ok = {0,0}, .x_okVersion = 0x0, .tasks_count = 2, .app_tasks = {&task_struct_sense_temp, &task_struct_temp_avg}, .initial_task = &task_struct_sense_temp, .final_task = &task_struct_temp_avg, .isActive = {0x0, 0x0}, .isActiveVersion = 0x0};	\
	app_struct_t __attribute__ ((persistent)) app_struct_app_3 = {.x_min = 1000000, .x_ok = {0,0}, .x_okVersion = 0x0, .tasks_count = 2, .app_tasks = {&task_struct_compress, &task_struct_send}, .initial_task = &task_struct_compress, .final_task = &task_struct_send, .isActive = {0x0, 0x0}, .isActiveVersion = 0x0};	\
	...
	task_struct_sense_temp.app_pointer = (void*) &(app_struct_app_2);	\
	task_struct_temp_avg.app_pointer = (void*) &(app_struct_app_2);	\
	task_struct_compress.app_pointer = (void*) &(app_struct_app_3);	\
	task_struct_send.app_pointer = (void*) &(app_struct_app_3);};
#define APP_ARRAY app_struct_t* __attribute__ ((persistent)) app_array[3] = {&app_struct_app_1, &app_struct_app_2, &app_struct_app_3};	\
	app_struct_t* __attribute__ ((persistent)) active_app_array[3] = {};	\
